...
trait Solver extends GameDef :
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
    ...
    neighbors.filterNot((b, _) => explored.contains(b))
  ...
  def from(initial: LazyList[(Block, List[Move])],
           explored: Set[Block]): LazyList[(Block, List[Move])] = {
    @tailrec
    def loop(initial: LazyList[(Block, List[Move])],
             result: LazyList[(Block, List[Move])],
             explored: Set[Block]): LazyList[(Block, List[Move])] = {
      if (initial.isEmpty) result
        ...
        initial match {
          case head #:: tail =>
            ...
            loop(tail ++ newNeighbors, result ++ LazyList(head), explored ++ newNeighbors.map(_._1).toSet)
        }
      }
    }

    loop(initial, LazyList.empty, explored)
  }
   ...
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = pathsFromStart.filter(pwh => done(pwh._1))
