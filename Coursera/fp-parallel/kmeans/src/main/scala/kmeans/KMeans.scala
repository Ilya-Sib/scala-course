...
class KMeans extends KMeansInterface :
  ...
  def classify(points: ParSeq[Point],
               means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] = {
    val grouped = points.groupBy(p => findClosest(p, means))

    (for (mean <- means)
      yield (mean, grouped.getOrElse(mean, ParSeq.empty[Point])))
      .to(ParMap)
  }
  ...
  def update(classified: ParMap[Point, ParSeq[Point]], oldMeans: ParSeq[Point]): ParSeq[Point] =
    for (mean <- oldMeans)
      yield findAverage(
        mean,
        classified.getOrElse(mean, ParSeq.empty[Point])
      )

  def converged(eta: Double, oldMeans: ParSeq[Point], newMeans: ParSeq[Point]): Boolean =
    newMeans.zip(oldMeans).forall((newMean, oldMean) => oldMean.squareDistance(newMean) <= eta)
